...
{
    // epoll 监听的事件类型 epoll-in代表可读事件 epoll-et：电平触发，变化时提示一次，减少重复通知
...
auto TCPServer::listen(const std::string& iface, int port) -> void
{
    ...
    // 把socket变成监听socket，监听指定的iface和port
    // connect -> bind -> listen (is_listening=true)
...
auto TCPServer::poll() noexcept -> void
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (event.events & EPOLLIN) {
            if (socket == &listener_socket_) {
                ...
                have_new_connection = true; // just flag, not accept
                continue;
            }
    ...
    while (have_new_connection) {
        ...
        socket->recv_callback_ = recv_callback_; // 原先注册的recv cb
        // 加入epoll list，监听事件
