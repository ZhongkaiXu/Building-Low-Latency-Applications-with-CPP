...
public:
    ...
    int epoll_fd_ = -1; // epoll实例，check是否有未完成事件
    TCPSocket listener_socket_; // 监听是否有client连接的socket

    epoll_event events_[1024]; // epoll临时数组：待处理事件（fd+type）

    /// Collection of all sockets, sockets for incoming data, sockets for outgoing data and dead connections.
    // 一轮epoll，recv到的client msg的client socket
