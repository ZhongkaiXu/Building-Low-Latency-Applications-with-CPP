...
auto TCPSocket::connect(const std::string& ip, const std::string& iface, int port, bool is_listening) -> int
{
    ...
    socket_attrib_.sin_addr.s_addr = INADDR_ANY; // IP地址：任意
    socket_attrib_.sin_port = htons(port); // 端口号：htons()将主机字节序转换为网络字节序
    socket_attrib_.sin_family = AF_INET; // 地址族：IPv4
